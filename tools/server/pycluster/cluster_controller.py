...
class ClusterRunScriptHandler(ClusterControllerHandler):

	def __init__(self, script_file, run_type):
		"""

		:param script_file: script.py name
		:param run_type: base, cell, all
		:return:
		"""
		super().__init__(0)
		self.script = open(script_file).read()
	...
	def do(self):
		...
		def f(info):
			if self.run_type == 'all':
				return True
			elif self.run_type in info.componentName:
				return True
			else:
				return False

		for value in self.interfaces_groups.values():
			processes = [v for v in value if f(v)]
			self.runscript(processes, self.script)


	def runscript( self, procs, script = None, lockCells = False, outputs = None,
				   prefix = True ):
		import telnetlib
		...
		for p in procs:
			...
			try:
				if not p.extradata3:
					continue
				t = telnetlib.Telnet( p.intaddr, int(p.extradata3 ), timeout=1)
				conns[t] = p
			except:
				print("Cannot connect to:", p.intaddr, int(p.extradata3 ))

		if script: script = StringIO( script )
		# Macro to read server output
		def slurp( conn, line = "", silent = False, output = sys.stdout ):

			index, match, s = conn.expect( [b">>> ", b"\.\.\. "] )
			more = (index == 1)
		...
		def sendAndRecv( line ):
			more = False
			for conn, p in conns.items():
				conn.write( line.encode("ascii") )
				more = slurp( conn, line, output = sys.stdout ) or more

			return more

		for conn, p in conns.items():
			conn.set_debuglevel(10)
			conn.expect([b"password:"], timeout=10)
			conn.write(b"a\r\n")
...
if __name__ == "__main__":
	...
	if len(sys.argv)  >= 2:
		...
		# 执行脚本
		elif cmdType == "runscript":
			# script 脚本名称
			# run_type : base, cell, all
			script, run_type = sys.argv[2], sys.argv[3]
			clusterHandler =ClusterRunScriptHandler(script, run_type)
		else:
			uid = -1
