...
inline bool loadConfig()
{
	...
	if (KBEngine::g_appPublish == 0)
	{
		g_kbeSrvConfig.loadConfig("server/kbe-dev.xml");
	}
	else
	{
		g_kbeSrvConfig.loadConfig("server/kbe-publish.xml");
	}

	g_kbeSrvConfig.loadConfig("server/kbe.xml");
