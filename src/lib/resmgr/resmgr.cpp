...
void Resmgr::autoSetPaths()
{
	...
	s = s.substr(0, pos1 + 1);
	kb_env_.root_path = s;
	kb_env_.res_path = kb_env_.root_path + "/kbengine/res/;" + kb_env_.root_path + "/server/res/;" + kb_env_.root_path + "/server/res/entities/;" + kb_env_.root_path + "/server/res/server";
}
...
std::string Resmgr::getPyUserResPath()
{
	...
	if(respath == "")
	{
		respath = matchRes("server/kbe.xml");
		std::vector<std::string> tmpvec;
		tmpvec = KBEngine::strutil::kbe_splits(respath, "server/kbe.xml");
...
std::string Resmgr::getPyUserAssetsPath()
{
	...
	if (path == "")
	{
		...
		strutil::kbe_replace(path, "/entities", "");
		strutil::kbe_replace(path, "\\entities", "");
	}
